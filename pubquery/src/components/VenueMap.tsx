...
// Hardcoded coordinates for common Stockholm student venues
const KNOWN_VENUE_COORDS: Record<string, [number, number]> = {
  'brazilia': [59.3497, 18.0695],
  'kårhuset': [59.3476, 18.0732],
  'nymble': [59.3476, 18.0732],
  'södra husen': [59.3626, 18.0630],
  'aula magna': [59.3640, 18.0645],
  'allhuset': [59.3645, 18.0620],
  'nobelparken': [59.3600, 18.0200],
  'flemingsberg': [59.3900, 17.9400],
  'handels': [59.3385, 18.0620],
  'valhallavägen': [59.3476, 18.0732],
  'kista': [59.4030, 17.9480],
  'telefonplan': [59.3150, 18.0300],
  'hötorget': [59.3345, 18.0600],
  'mosebacke': [59.3170, 18.0700],
  'reaktorhallen': [59.3497, 18.0695],
  'kth': [59.3476, 18.0732],
  'stockholms universitet': [59.3640, 18.0645],
  'karolinska institutet': [59.3600, 18.0200],
  'södertörns högskola': [59.3900, 17.9400],
  'konstfack': [59.3150, 18.0300],
}

// Geocoding utility using OpenStreetMap Nominatim API (free, no API key required)
export async function geocodeVenue(venueName: string, location: string): Promise<[number, number] | null> {
  // First check if we have hardcoded coordinates
  const searchTerms = [
    venueName.toLowerCase(),
    location.toLowerCase(),
    `${venueName.toLowerCase()} ${location.toLowerCase()}`
  ]
  
  for (const term of searchTerms) {
    for (const [key, coords] of Object.entries(KNOWN_VENUE_COORDS)) {
      if (term.includes(key) || key.includes(term)) {
        console.log(`Found hardcoded coords for: ${venueName} (${key})`)
        return coords
      }
    }
  }
  ...
  try {
    const query = `${venueName}, ${location}, Stockholm, Sweden`
    ...
    if (data && data.length > 0) {
      ...
      console.log(`Geocoded ${venueName} to: ${lat}, ${lon}`)
      return [lat, lon]
    }
...
// Batch geocode venues with concurrent requests for faster loading
export async function geocodeVenues(
  venues: Array<{ venue_name: string; location: string; maps_link?: string | null }>
): Promise<Map<string, [number, number]>> {
  const coordinatesMap = new Map<string, [number, number]>()
  
  // Process all venues concurrently for faster loading
  const geocodePromises = venues.map(async (venue) => {
    const key = `${venue.venue_name}-${venue.location}`
    
    // Try to extract coordinates from maps_link if it's a Google Maps link with coordinates
    if (venue.maps_link) {
      const coords = extractCoordsFromMapsLink(venue.maps_link)
      if (coords) {
        return { key, coords }
      }
    }
    ...
    const coords = await geocodeVenue(venue.venue_name, venue.location)
    if (coords) {
      return { key, coords }
    }
  ...
  for (const result of results) {
    if (result) {
      coordinatesMap.set(result.key, result.coords)
    }
  }
  
  console.log(`Geocoded ${coordinatesMap.size} out of ${venues.length} venues`)
  
  return coordinatesMap
}

// Try to extract coordinates from a Google Maps link
function extractCoordsFromMapsLink(url: string): [number, number] | null {
  try {
    // Google Maps links with coordinates look like:
    // https://www.google.com/maps/place/@59.3293,18.0686,15z
    // or https://www.google.com/maps?q=59.3293,18.0686

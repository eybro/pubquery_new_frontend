...
function getAllowedGuestsString(code: string) {
  switch (code) {
    case 'all_students':
      return 'alla studenter'
    case 'members':
      return 'sektionsmedlemmar'
    case 'members_plus_one':
      return 'sektionsmedlemmar + 1 gäst'
    case 'kmr':
      return 'KMR-medlemmar'
    case 'everyone':
      return 'alla'
    default:
      return code
...
export default function DinnerCard({ dinner, onClick }: Props) {
  ...
  return (
    ...
    >
      ...
      <div>
        {isOpen ? (
          <p className="text-sm text-blue-700 italic mt-1 font-semibold">Pågår nu</p>
        ) : (
